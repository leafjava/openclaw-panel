"""
保存 Chrome 新闻资讯数据到数据库（接收 Claude Code Skill 传入的 JSON）
用法: python manage.py save_chromeNews --json '[{...}]'
"""
import json
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stdout.write(self.style.SUCCESS(
                f'Chrome 新闻: 新增 {new_count} 条（评分 ≥ 5），共处理 {len(data)} 条'
            ))
